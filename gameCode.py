...
class Button: 
    ...
    def buttonClicks(self,world, mouseX, mouseY,button):
        if world.screen == self.screen: 
            if mouseX > self.xPercentage / 100 * getWindowWidth() - self.width / 2 and mouseX < self.xPercentage / 100 * getWindowWidth() - self.width / 2 + self.width:
                if mouseY > self.yPercentage / 100 * getWindowHeight() - self.height / 2 and mouseY <  self.yPercentage / 100 * getWindowHeight() - self.height / 2 + self.height:
    ...
    def drawButton(self,world):
        print(1)
        #w, h = sizeString(self.text, self.text_size)
        w = 10 
        h = 10
        print(2)
        if w > self.width: 
        ...
        if world.screen == self.screen: 
            print(4)
            fillRectangle(self.xPercentage / 100 * getWindowWidth() - self.width / 2 , self.yPercentage / 100 * getWindowHeight() - self.height / 2 ,self.width, self.height, self.button_background_color)
            print(4.2)
            print(self.text, self.xPercentage / 100 * getWindowWidth() - w / 2, self.yPercentage / 100 * getWindowHeight() - h / 2 ,self.text_size)
            #drawString(self.text, self.xPercentage / 100 * getWindowWidth() - w / 2, self.yPercentage / 100 * getWindowHeight() - h / 2 ,self.text_size)
            print(4.5)
            #drawRectangle(self.xPercentage / 100 * getWindowWidth() - self.width / 2 , self.yPercentage / 100 * getWindowHeight() - self.height / 2 ,self.width, self.height,self.button_outline_color) 
            print(4.9)
            print(5)
            ...
            for i in self.fancyButtons:
                if world.mode == i:
                    if self.button_name == i:
                        fillRectangle(self.xPercentage / 100 * getWindowWidth() - self.width / 2 , self.yPercentage / 100 * getWindowHeight() - self.height / 2 ,self.width, self.height, "lime")
                        drawString(self.text, self.xPercentage / 100 * getWindowWidth() - w / 2, self.yPercentage / 100 * getWindowHeight() - h / 2 ,self.text_size)
                        drawRectangle(self.xPercentage / 100 * getWindowWidth() - self.width / 2 , self.yPercentage / 100 * getWindowHeight() - self.height / 2 ,self.width, self.height,"green") 
...
class Compare: 
    ...
    def drawImages(self,world): 
        if world.screen == "game":
            drawImage(self.compare_1.image,25 / 100 * getWindowWidth(),35 / 100 * getWindowHeight())
            ...
            drawImage(self.compare_2.image,75 / 100 * getWindowWidth(),35 / 100 * getWindowHeight())
            ...
            if world.mode == "easy":
                if self.tries == 3:
                    drawImage(world.lifeAlive, getWindowWidth() / 2 + 100, 150)
                    drawImage(world.lifeAlive, getWindowWidth() / 2, 150)
                    drawImage(world.lifeAlive, getWindowWidth() / 2 - 100, 150)
                if self.tries == 2:
                    drawImage(world.lifeDead, getWindowWidth() / 2 + 100, 150)
                    drawImage(world.lifeAlive, getWindowWidth() / 2, 150)
                    drawImage(world.lifeAlive, getWindowWidth() / 2 - 100, 150)
                if self.tries == 1:
                    drawImage(world.lifeDead, getWindowWidth() / 2 + 100, 150)
                    drawImage(world.lifeDead, getWindowWidth() / 2, 150)
                    drawImage(world.lifeAlive, getWindowWidth() / 2 - 100, 150)
...
def startWorld(world):
    ...
    for i in [("art_data.csv","https://raw.githubusercontent.com/ryanbleh/higherorlower/main/art_data.csv"),("dead life.png","https://github.com/ryanbleh/higherorlower/blob/main/dead%20life.png?raw=true"),("alive life.png","https://github.com/ryanbleh/higherorlower/blob/main/alive%20life.png?raw=true"),
    ...
    print("test4")
    world.lifeAlive = loadImage("alive life.png", scale= .2)
    world.lifeDead = loadImage("dead life.png", scale= 0.2)
    ...
    world.screen = "start"
    world.buttons_info = [[50,60,300,100,"startGame","start","Start Game", 70,],[50,75,300,100,"highScore","start","High Scores",70],[50,90,300,100,"modes","start","Modes",70],[75,70,200,50,"higher","game","Higher",40],[75,78.5,200,50,"lower","game","Lower",40],[50,10,300,230,"scores","game"," ",40],[50,0,300,200,"scores","game"," ",40],[43.01,60,200, 100, "restart","fail","Restart",40],[56.91,60,200, 100, "mainmenu","fail","Back to Menu",40],[50,71.8,402,100,"save","fail","Save score to Leaderboard",40],[50,83.5,402,100,"copy","fail","Copy Score to Clipboard",40],[8.5,93,200,100,"back","leaderboard","Back",60],[25,50,300, 600, "easy","modes","Easy",60],[50,50,300, 600, "normal","modes","Normal",60],[75,50,300, 600, "hard","modes","Hard",60],[8.5,93,200,100,"back","modes","Back",60],[82,25,300,100,"easylb","leaderboard","Easy",40],[82,50,300,100,"normallb","leaderboard","Normal",40],[82,75,300,100,"hardlb","leaderboard","Hard",40]]
    world.buttons = [ ]
    #args are posX,posY,screen,string,size,color="black",bold=False,italic=False
    world.strings_info = [[50,25,"start","Higher or Lower | Art Edition",70,],[50,25,"fail","You scored:",100],[50,40,"save","Type Your Initials:",100,"black"],[50,60,"save","Press Enter once ready",30]]
    world.strings = [ ]
    print("test8")
...
def drawWorld(world):
    ...
    if world.draw == True:
        print("hiu")
        if world.bigX == True:
            drawImage(world.lifeAlive,getWindowWidth() / 2,getWindowHeight() / 2,scale=world.bigXscale)
        print("test18")
        if world.screen == "fail":
            if world.error == True:
                at_w, at_h = sizeString("Error: Score is too low to save ",40)
                drawString("Error: Score is too low to save", 50 / 100 * getWindowWidth() - at_w / 2,50 / 100 * getWindowHeight() - at_h / 2,size=40,color="red")
        print("test19")
        if world.screen == "save":
        ...
        if world.screen == "leaderboard":
            for i in world.leaderboard_background:
                i.drawImg()
                fillRectangle(getWindowWidth() / 2 - 200, 0 + 35, 400,800,"white")
            for i in range(1,10):
                drawLine(getWindowWidth() / 2 - 200,i * 80 + 35,getWindowWidth() / 2 + 200,i * 80 + 35)
            if world.modelb == "easylb":
                for i in range(len(world.easy_names)):
                    drawString(world.easy_names[i], getWindowWidth() / 2 - 170, i * 80 + 40 - 10 + 35, 40 )
                    drawString(world.easy_scores[i], getWindowWidth() / 2 + 150, i * 80 + 40 - 10 + 35, 40 )
            print("test21")
            if world.modelb == "normallb":
                for i in range(len(world.normal_names)):
                    drawString(world.normal_names[i], getWindowWidth() / 2 - 170, i * 80 + 40 - 10 + 35, 40 )
                    drawString(world.normal_scores[i], getWindowWidth() / 2 + 150, i * 80 + 40 - 10 + 35, 40 )
            print("test22")
            if world.modelb == "hardlb":
                for i in range(len(world.hard_names)):
                    drawString(world.hard_names[i], getWindowWidth() / 2 - 170, i * 80 + 40 - 10 + 35, 40 )
                    drawString(world.hard_scores[i], getWindowWidth() / 2 + 150, i * 80 + 40 - 10 + 35, 40 )
        print("test23")
        for i in world.buttons:
            i.drawButton(world)
        print("test24")
        ...
        if world.screen == "game":
            if world.mode == "hard":
                drawString(round(world.end - world.start,2),getWindowWidth() / 2 - 60, 120,100)
        if world.screen == "fail":
